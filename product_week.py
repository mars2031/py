...
if not os.path.exists(fuck_file):
    ...
    total = []
    for row in data:
        if row['org_pri'] == 'N/A' or row['exchange_rate'] == 'N/A' or row['sku_number'] == 'N/A':
            continue
        if not row['org_pri'] or not row['exchange_rate'] or not row['sku_number']:
            continue
        rb = []
    ...
    sql = "update sales_report_info set sales_amount = %s,sales_cost = %s,freight_cost = %s,profit_amount = %s where category_id = %s and md5id = %s"
