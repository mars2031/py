...
pwd = os.getcwd()
folder = pwd + '/' + date
fuck_file = folder + '/' + date + '_fuck.txt'
print folder
...
if not os.path.exists(fuck_file):
    ...
    cursor.execute("select entity_id,attribute_set_id from table_auto_order order by value desc")
    ...
    sql = "update table_auto_order set updated_at= %s where entity_id = %s"
